  ...
  switch (isNumeric(colName)) {
    case true:
      if (colName === "totalValue") {
        if (direction === "asc") {
          return stocksObj.sort(
            (a, b) =>
              a["numShares"] * a["currPrice"] - b["numShares"] * b["currPrice"]
          );
        } else {
          return stocksObj.sort(
            (a, b) =>
              b["numShares"] * b["currPrice"] - a["numShares"] * a["currPrice"]
          );
        }
    ...
    case false:
      if (colName === "watchlist") {
        if (direction === "asc") {
          return stocksObj.sort(
            (a, b) => inWatchlist(watchlist, a) - inWatchlist(watchlist, b)
          );
        } else {
          return stocksObj.sort(
            (a, b) => inWatchlist(watchlist, b) - inWatchlist(watchlist, a)
          );
        }
      }
    ...
    switch (isNumeric(filter.columnValue)) {
      case true:
        if (filter.criteria === "equals") {
          ...
          filteredStocks.forEach((stock) => {
            if (
              stock[filter.columnValue] >= Number(filter.lowerLimit) &&
